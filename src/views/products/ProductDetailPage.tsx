    ...
    <div className="bg-[#0A0A0E] text-slate-100 min-h-screen">
      ...
      <section className="pt-28 pb-4 bg-[#0A0A0E] border-b border-red-950/60">
        <div className="site-container flex items-center gap-2 text-xs text-slate-400 font-mono">
          ...
          <Link href="/products" className="hover:text-red-400 transition-colors">Product</Link>
          <ChevronRight className="w-3 h-3 text-slate-600" />
          <span className="text-white font-bold truncate max-w-xs">{product.name}</span>
        </div>
      </section>

      {/* ── Monumental Editorial Header (Centered Layout) ── */}
      <section className="pt-10 pb-12 bg-[#0A0A0E] relative overflow-hidden">
        ...
        <div className="site-container relative z-10 space-y-5 text-center flex flex-col items-center">
          <div className="flex flex-wrap items-center justify-center gap-3">
            <span className="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-full text-xs font-mono font-bold uppercase tracking-wider bg-red-950/80 text-rose-300 border border-red-500/40 shadow-xs">
              {isSolarPump ? <Tractor className="w-3.5 h-3.5 text-rose-400" /> : <Sun className="w-3.5 h-3.5 text-amber-400" />}
              <span>{product.categoryLabel}</span>
            </span>
            <span className="inline-flex items-center gap-1.5 text-xs text-emerald-400 font-mono font-bold px-3.5 py-1.5 rounded-full bg-slate-900/80 border border-red-950/60 shadow-xs">
          ...
          <p className="text-red-400 font-mono text-sm sm:text-base font-bold max-w-3xl text-center">
            {product.tagline}
          </p>
      ...
      <section className="py-14 bg-[#0A0A0E] border-t border-red-950/60">
        <div className="site-container space-y-12">
          ...
          <div className="pt-6 border-t border-red-950/60 flex flex-wrap items-center justify-center gap-4">
            ...
            <Link
              href={`/contact?subject=${encodeURIComponent(`Engineering Inquiry: ${product.name}`)}`}
